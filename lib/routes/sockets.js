...
exports.init = function (app, sessionSockets, server, ioLogLevel) {
    ...
    function checkSocketName(name) {
        if (socketNames.indexOf(name) === -1) {
            throw new generalUtils.DFLCondition(generalUtils.DFLCondition.fatal, 'Invalid socket named ' + name);
        }
    }
    ...
    var getSocket = function (session, socketName) {
        ...
        if (typeof session === 'undefined') {
            throw new generalUtils.DFLCondition(generalUtils.DFLCondition.trans, 'No session exists');
        }
        var sessionSockets = socketCache[session.id];
        if (typeof sessionSockets === 'undefined') {
            throw new generalUtils.DFLCondition(generalUtils.DFLCondition.trans, 'Session not cached');
        }
        var socket = sessionSockets[socketName];
        if (!socket) {
            throw new generalUtils.DFLCondition(generalUtils.DFLCondition.trans,
                'Failed to find expected socket named ' + socketName + ' for session id ' + sessionSockets.id);
        }
        return socket;
    };
    ...
    function addSocketToSession(socketName, session, socket) {
        ...
        if (typeof session === 'undefined' || typeof socket === 'undefined') {
            throw new generalUtils.DFLCondition(generalUtils.DFLCondition.fatal, 'No session and/or socket passed');
        }

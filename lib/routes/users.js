...
module.exports = function users(app) {
    ...
    if (!session || !db || !hogan) {
        throw new generalUtils.DFLCondition('fatal', 'System not properly initialized');
    }
    ...
    app.namespace('/session', function () {
        ...
        app.get('/', function (req, res) {
            var s = req.session, ack = new generalUtils.DFLCondition('ack');
            if (s && s.username) {
        ...
        app.put('/', function (req, res) {
            ...
            if (!body.name || !body.password) {
                res.json(new generalUtils.DFLCondition('fatal', 'Missing username and/or password params'));
            } else {
                    ...
                    function (err, userRec) {
                        if (err) {
                            ...
                            if (userRec && userRec.username === body.name && userRec.password === body.password) {
                                ...
                                res.json(new generalUtils.DFLCondition('error', "Username and/or password don't match any user")); // i18n
                            }
                        }
                    });
            }
        ...
        app.get('/signoff', function (req, res) {
            ...
            res.json(new generalUtils.DFLCondition('ack', 'User signed off'));
        });
        ...
        app.post('/', function (req, res) {
            ...
            if (!body.name || !body.password || !body.email) {
                res.json(new generalUtils.DFLCondition('error', 'Missing required fields'));
            } else { // TODO encrypt password
                db.singupUser({username: body.name, password: body.password, email: body.email},
                    function (err, doc) {
                        if (err) {
                            ...
                            res.json(new generalUtils.DFLCondition('ack', 'Signed up user ' + body.name));
                        }
                    });
            }
        });
...
//            } catch (err) {
//                throw new generalUtils.DFLCondition('trans', err);
//            }

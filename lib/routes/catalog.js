    ...
    app.namespace('/note', function () {
        ...
        app.post('/', function (req, res, next) {
            if (req.body.note) {
                ...
                res.json({type: 'fatal', msg: 'missing note'});
            }
        });
        ...
        app.get('type/json', function (req, res, next) {
            var query = req.query
            if (!query) {
                res.json({type: 'fatal', msg: 'No note type query found'});
            } else {
            ...
            if (req.query.id) {
                workService.getChunk(req.query.id, function (err, chunk) {
                    if (err) {
                        ...
                        res.json({type: 'ack', chunk: chunk});
                    }
                });
            } else {
                res.json({type: 'fatal', msg: 'missing chunk id'});
            }
        });

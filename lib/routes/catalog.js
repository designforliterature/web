...
var
// The schema (class definitions, etc)
    schema = require('../model/schema.js').schema,

// The catalog service
    catalogService = require('../services/catalogService.js').catalogService,

// The work service
    workService = require('../services/workService.js').workService,

// The person service
    personService = require('../services/personService.js').personService,
...
// db: Require MongoDB implementation
    mongodb = require('../db/mongoDbManager.js');
...
function makeCanonicalCatalog(metadata, txId) {
    ...
    if (typeof workType === 'undefined') {
        throw new generalUtils.DFLCondition(generalUtils.DFLCondition.fatal, 'Work type is required', {txId: txId});
    }
    ...
    for (fieldId in catalog) {
        ...
        if (typeof fieldSpec === 'undefined') {
            throw new generalUtils.DFLCondition(generalUtils.DFLCondition.fatal, 'No field spec "' + fieldId + '"');
        }
        if (fieldSpec.required) {
            var val = catalog[fieldId];
            if (typeof val === 'undefined' || val.length === 0) {
                throw new generalUtils.DFLCondition(generalUtils.DFLCondition.error, 'Catalog field "' + fieldSpec.name + '" is required', {txId: txId});
            }
        }
        var validator = schema.validators[fieldSpec.validator];
        if (typeof validator === 'undefined') {
            throw new generalUtils.DFLCondition(generalUtils.DFLCondition.fatal, 'No validator for catalog field ' + fieldSpec.name, {txId: txId});
        }
    ...
    for (fieldId in catalog) {
        if (typeof catalog[fieldId] !== 'undefined') {
            ...
            if (typeof xformer === 'undefined') {
                throw new generalUtils.DFLCondition(generalUtils.DFLCondition.fatal, 'No transformer for catalog field ' + fieldSpec.name, {txId: txId});
            }
    ...
    app.namespace('/note', function () {
        ...
        app.post('/', function (req, res, next) {
            if (req.body.note) {
                ...
                res.json(new generalUtils.DFLCondition(generalUtils.DFLCondition.fatal, 'missing note param'))
            }
        });
        ...
        app.get('type/json', function (req, res, next) {
            var query = req.query
            if (!query) {
                res.json(new generalUtils.DFLCondition(generalUtils.DFLCondition.fatal, 'No note type query found'));
            } else {
    ...
    app.namespace('/catalog', function () {
        ...
        app.get('work/chunk', function (req, res, next) {
            if (req.query.id) {
                ...
                res.json(new generalUtils.DFLCondition(generalUtils.DFLCondition.fatal, 'missing chunk id param'));
            }
        });

        /**
         * Search for a person by name query. The query.name request parameter is a string.
        ...
        app.get('persons/json', function (req, res, next) {
            var query = req.query
            if (!query) {
                res.json(new generalUtils.DFLCondition(generalUtils.DFLCondition.fatal, 'Missing query param'));
            } else if (!query.name) {
                res.json(new generalUtils.DFLCondition(generalUtils.DFLCondition.ack, 'No results', {persons: []}));
            } else {
                personService.search(query.name, function (error, result) {
        ...
        /**
         * Submit a search query on the catalog.
         */
        app.post('/search/query', function (req, res, next) {
            ...
            try {
                if (!query) {
                    res.json(new generalUtils.DFLCondition(generalUtils.DFLCondition.ack, 'No results found.'));
                } else {
...
//            }
        ...
        /**
         * Submit a catalog item and optionally its contents.
         */
        app.post('/submit', function (req, res) {

            if (!req.session || !req.session.username) {
                res.json(new generalUtils.DFLCondition(generalUtils.DFLCondition.error, 'User not signed in'));
            } else {
                ...
                try {
                    ...
                    catalogService.saveMetadata(req.session, catalog, function (error, saveMetadataResult) {
                        if (error) {
                            ...
                            if (!catalogId) {
                                var err = new generalUtils.DFLCondition(generalUtils.DFLCondition.fatal, 'Catalog id not returned', {txId: txId});
                                ...
                                if (haveContent) {
                                    ...
                                    fs.readFile(filepath, {encoding: "UTF-8"}, function (err, content) {
                                        ...
                                        try {
                                            ...
                                            catalogService.saveContent(req.session, catalog, function (error) {
                                                var result = error;
                                                result.txId = txId;
                    ...
                    res.json(new generalUtils.DFLCondition(generalUtils.DFLCondition.ack, null, {txId: txId})); // send response immediately (errors reported later through tx socket)

...
exports.makeDbUrl = function makeDbUrl(config, dbName) {
    try {
        ...
        throw new generalUtils.DFLCondition('fatal', 'DB error makeDbUrl: ' + err);
    }
};
...
exports.use = function (expressApp, cb) {
    ...
    function createConnection(config, dbName, cb) {
        try {
            ...
            throw new generalUtils.DFLCondition('fatal', 'DB error createConnection: ' + err);
        }
    }
    ...
    try {
        ...
        throw new generalUtils.DFLCondition('fatal', err);
    }
};
...
function createCatalog(session, catalog, callback) {
    ...
    getCatalogCol().insert(catalog, insertUpdateOptions, function (err, doc) {
        if (err) {
            callback(new generalUtils.DFLCondition('fatal', 'Failed to create catalog item. ' + err.toString()));
        } else { // successful insert
            var catalogId = doc[0]._id.toString(),
                title = doc[0].title;
            callback(null,
                new generalUtils.DFLCondition('ack',
...
//    }
    ...
    getCatalogCol().update(query, {$set: catalog}, insertUpdateOptions, function (err, count) {
        if (err) {
            ...
            callback(new generalUtils.DFLCondition('trans',
                'Failed to update catalog item id ' + catalogId,
                err));
        } else if (count > 0) { // successful update
            callback(null, new generalUtils.DFLCondition('ack',
        ...
        collection.find({$text: {$search: queryString}}, (fieldsToReturn || {}))
            ...
            .toArray(function (error, results) {
                if (error) {
                    ...
                    var r = new generalUtils.DFLCondition('ack', results.length + ' results found');
...
var saveCatalogMetadata = function (session, catalog, callback) {
    try {
        ...
        callback(new generalUtils.DFLCondition('fatal', 'Failed to save catalog metadata. ', err));
    }
};
    ...
    if (catalog.content) {
        if (catalog._id) {
            getWorksCol().findOne({_id: catalog._id}, function (err, doc) {
                if (err) {
                    callback(new generalUtils.DFLCondition('error', 'Cannot find content for catalog id ' + catalog._id, err));
            ...
            callback(new generalUtils.DFLCondition('error', 'Content missing catalog id'));
        }
    } else {
        callback(new generalUtils.DFLCondition('error', 'Missing content'));
    }
};
...
function writeToplevelChunk(catalog, firstChunk, lastChunk, toc, chunkProcessStack) {
    ...
    topChunk.id = firstChunk ? topChunk.catalogId : generalUtils.makeUUID();
...
function findUserByUsername(username, callback) {
    getUserCol().findOne({username: username}, function (err, doc) {
        if (err) {
            callback(new generalUtils.DFLCondition('trans', 'Failed to find user. ', err));
        } else {
            callback(null, doc);
        }
    });
...
function singupUser(userRec, callback) {
    if (userRec) {
        getUserCol().insert(userRec, function (err, docs) {
            if (err) {
                callback(new generalUtils.DFLCondition('trans', 'Failed to sign up user. ', err));
            } else {
                callback(null, docs);
            }
        });
    } else {
        callback(new generalUtils.DFLCondition('fatal', 'missing user record'));
    }
}
...
var getChunk = function (chunkId, callback) {
    if (chunkId) {
        getWorksCol().findOne({_id: chunkId}, function (err, chunk) {
            if (err) {
                callback(new generalUtils.DFLCondition('error', 'Content for work chunk id ' + chunkId + ' not found', err));
            } else {
                callback(null, new generalUtils.DFLCondition('ack', null, {chunk: chunk}));
            }
        });
    } else {
        callback(new generalUtils.DFLCondition('fatal', 'missing chunk id param'));
    }
};
            ...
            getWorksCol().findAndModify(
                ...
                function (err, chunk) {
                    if (err) {
                        ...
                        callback(null, new generalUtils.DFLCondition('ack', null, {updated: chunk ? true : false}));
                    }
                }
            );
        } catch (err) {
            callback(new generalUtils.DFLCondition('fatal', 'Failed to add note.', err));
        }
    } else {
        callback(new generalUtils.DFLCondition('fatal', 'missing note param'));
    }
};

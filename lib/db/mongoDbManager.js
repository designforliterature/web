 ...
 */
var userCol, worksCol, catalogCol, personCol;
...
function findUserByUsername(username, callback) {
    getUserCol().findOne({username: username}, function (err, doc) {
        if (err) {
            callback(appUtils.makeError({type: 'trans', msg: 'Failed to find user. '}, err));
        } else {
            callback(null, doc);
        }
    });
...
function singupUser(userRec, callback) {
    getUserCol().insert(userRec, function (err, docs) {
        if (err) {
            callback(appUtils.makeError({type: 'trans', msg: 'Failed to sign up user. '}, err));
        } else {
            callback(null, docs);
        }
    });

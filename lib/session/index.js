...
var SessionSockets = function (io, sessionStore, cookieParser, sessionKey) {
    if (!sessionKey) {
        throw new generalUtils.DFLCondition(generalUtils.DFLCondition.fatal, 'Missing session key');
    }

    this.getSession = function (socket, callback) {
        if (!socket) {
            throw new generalUtils.DFLCondition(generalUtils.DFLCondition.fatal, 'socket is not defined');
        }
        cookieParser(socket.handshake, {}, function (parseErr) {
            if (!socket) {
                callback(new generalUtils.DFLCondition(generalUtils.DFLCondition.trans, 'socket not defined in cookieParser'));
                return;
            }
            ...
            if (!cookie) {
                callback(new generalUtils.DFLCondition(generalUtils.DFLCondition.trans, 'Could not find cookie session id ' + sessionKey));
                return;
            }
            sessionStore.load(cookie, function (storeErr, session) {
                var err = parseErr || storeErr || null;
                if (!err && !session) {
                    err = new generalUtils.DFLCondition(generalUtils.DFLCondition.trans, 'Could not lookup session by key: ' + sessionKey);
                }
...
var use = function (app, io, cookieParser, sessionKey) {
    ...
    var express = require('express'),
        ...
        url = require('../store/storeManager.js').makeDbUrl(config.db, config.db.sessionDb),
        store = new SessionStore({url: url});
    if (!store) {
        throw new generalUtils.DFLCondition(generalUtils.DFLCondition.fatal, 'Session store not created for url=' + url);
    }
...
exports.handle = function (req, res, next) {
    ...
    if (!sess) {
        throw new generalUtils.DFLCondition(generalUtils.DFLCondition.trans, 'Invalid session state: no session');
    } else {
        if (sess.views) {
            sess.views += 1;
        } else {
